...
class Creature implements Comparable<Creature>{
  ...
  void move(int heading, float option){
    ...
    if(option == 2){
      if((heading == 3) && (y != (Evo.grid_size - 1))){
        y += 1;
      } else if ((heading == 24) && (x != (Evo.grid_size - 1))){
        x += 1;
    ...
    Evo.creature_array[x][y] += 1;
    if((Evo.creature_array[x][y] > 0) && (Evo.monster_array[x][y] > 0)){
      kill();
  ...
  void eat(){
    ...
    if(strawb_present()){
      Evo.strawb_array[x][y]--;
      energy_level += 5;
      Evo.strawb_array[r.nextInt(Evo.grid_size)][r.nextInt(Evo.grid_size)]++;
    } else if(mushroom_present()){
      Evo.mushroom_array[x][y]--;
      kill();
      //System.out.println("Creature killed by mushroom");
      Evo.mushroom_array[r.nextInt(Evo.grid_size)][r.nextInt(Evo.grid_size)]++;
    }
  }
  ...
  int select_action(){
    ...
    if(nearest(Evo.mushroom_array) != 0){
      if(chromosome[2] != 0){
        actions_list.add(9);
      }
    }
    if(nearest(Evo.strawb_array) != 0){
      if(chromosome[3] != 0){
        actions_list.add(10);
      }
    }
    if(nearest(Evo.creature_array) != 0){
      if(chromosome[4] != 0){
        actions_list.add(11);
      }
    }
    if(nearest(Evo.monster_array) != 0){
      if(chromosome[5] != 0){
        actions_list.add(12);
      }
    }
  ...
  void do_action(int i){
    ...
    switch(i){
      ...
      case 1:
        ...
        break;
      case 2:
        move(nearest(Evo.mushroom_array), chromosome[2]);
        //System.out.println("Moving towards Mushroom");
        break;
      case 3:
        move(nearest(Evo.strawb_array), chromosome[3]);
        //System.out.println("Moving towards Strawberry");
        break;
      case 4:
        move(nearest(Evo.creature_array), chromosome[4]);
        //System.out.println("Moving towards Creature");
        break;
      case 5:
        move(nearest(Evo.monster_array), chromosome[5]);
